...
nameFile = f"Extracted Diversos horários(Abertura Fixa) - {str(datetime.now().timestamp()).replace('.','')}"
symbols = getSymbols()
f_StopGain = 0.009
f_StopLoss = 0
f_MinVolume = 100000
...
def verifyDate(date, reference = [2]):
                ...
                if verifyDate(dayOfWeek):
                    ...
                    for tick in ticks:
                        ...
                        if tick_date.hour == time_h and tick_date.minute == time_m:
                            variation = tick_close / open - 1
                            break
                    
                    variation = (close / open - 1) if (variation is None) else variation                
                    ocurrences += 1
