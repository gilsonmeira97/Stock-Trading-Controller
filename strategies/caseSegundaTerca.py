...
nameFile = f"Extracted Segunda-Terca - {str(datetime.now().timestamp()).replace('.','')}"
symbols = getSymbols()
f_StopGain = 0.016
f_StopLoss = -0.016
f_MinVolume = 100000
...
with open(f'extracteds/{nameFile}.csv', mode='w', newline='') as file:
    ...
    for i, symbol in enumerate(symbols):
        data_result = getDaysOfWeek(db, symbol, f_date_start, f_date_end, [1, 2], f_MinVolume)
        ...
        for data in datas:
            ...
            if (dayOfWeek == 1):
                last_object = data
            elif (last_object != None) and (dayOfWeek == 2):
                ocurrences += 1
