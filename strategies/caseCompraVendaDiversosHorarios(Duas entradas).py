...
nameFile = f"Extracted Diversos horários(Duas entradas) - {str(datetime.now().timestamp()).replace('.','')}"
symbols = getSymbols()
f_StopGain = 0.008
f_StopLoss = 0
f_MinVolume = 100000
                    ...
                    if verifyDate(dayOfWeek):
                        ...
                        for tick in ticks:
                            ...
                            if f_tick_reference is None and tick_date.hour == time_h and tick_date.minute == time_m:
                                ...
                                avg_price = (f_tick_reference['close'] + tick_close) / 2
                                continue
                            elif (avg_price != None) and ((tick_high / avg_price - 1) >= f_StopGain):
                        ...
                        ocurrences += 1
                        variation = (close / avg_price - 1) if (variation is None) else variation                
