...
nameFile = f"Extracted Diversos horários(Dois Dias) - {str(datetime.now().timestamp()).replace('.','')}"
symbols = getSymbols()
f_StopGain = 0.005
f_StopLoss = 0
f_MinVolume = 100000
    ...
    for i, symbol in enumerate(symbols):
        ...
        while not (time_h == 16 and time_m == 55) :
            ...
            for data in datas:
                ...
                if (tick_reference_0 != None) and open / tick_reference_0['close'] - 1 > f_StopGain:
                ...
                for tick in ticks:
                    ...
                    if (tick_reference_1 != None) and ((tick_high / tick_reference_1['close'] - 1) >= f_StopGain):
                        computarVariation(tick, f_StopGain)
                        tick_reference_1 = None
                    ...
                    if verifyDate(dayOfWeek) and (tick_reference_0 is None) and tick_date.hour == time_h and tick_date.minute == time_m:
                        ...
                        continue
                    elif verifyDate(dayOfWeek) and (tick_reference_1 is None) and tick_date.hour == time_h and tick_date.minute == time_m:
                        tick_reference_1 = tick
                        continue
                    
                if tick_reference_0 is None and tick_reference_1 is None:
                    continue

                if tick_reference_0 is not None and tick_reference_0['status'] is True:
                    computarVariation(tick_reference_0, close / tick_reference_0['close'] - 1)
                    tick_reference_0 = None
                
                if tick_reference_0 is None and tick_reference_1 is not None:
                    tick_reference_0 = tick_reference_1
                    tick_reference_1 = None
                
                if tick_reference_0 is not None:
                    tick_reference_0['status'] = True
